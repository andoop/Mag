...
class RepositoryStatus {
  ...
  String toString() {
    ...
    if (currentBranch != null) {
      ...
      final shortCommit =
          currentCommit!.length > 8 ? currentCommit!.substring(0, 8) : currentCommit;
...
class StatusOperation {
  ...
  Future<RepositoryStatus> status() async {
    ...
    final indexPaths = <String, IndexEntry>{};
    ...
    return RepositoryStatus(
      staged: _sortEntries(staged),
      unstaged: _sortEntries(unstaged),
      untracked: _sortEntries(untracked),
    ...
    await for (final entity in dir.list(recursive: true)) {
      if (entity is File) {
        ...
        try {
          ...
          files[relativePath] = repo.objects.hashBlob(content);
        } on FileSystemException {
          // Skip unreadable files so status can still be computed on platforms
          // that expose entries which cannot be opened through dart:io.
        }
      }
    }
  ...
  bool _shouldIgnore(String relativePath) {
    ...
    return p.split(relativePath).contains('.git');
  }

  List<StatusEntry> _sortEntries(List<StatusEntry> entries) {
