    ...
    if (hasEditTools) {
      lines.addAll(zh
          ? [
              ...
              '',
              '## 规则 2：`write` 只用于新文件（绝对要求）',
              '`write` 工具只能用于创建新文件。如果文件已经存在，你必须使用 `edit` 或 `apply_patch`。',
              '如果 `write` 报错说"文件已存在"，你必须改用 `edit` 或 `apply_patch`，绝不能再次调用 `write`。',
              '',
              '## 规则 3：每次编辑后重新读取',
          ...
          : [
              ...
              '',
              '## Rule 2: `write` is for NEW files only (absolute requirement)',
              '`write` creates new files only. If a file already exists, you MUST use `edit` or `apply_patch`.',
              'If `write` returns "file already exists", switch to `edit` or `apply_patch`. NEVER call `write` again on that file.',
              '',
