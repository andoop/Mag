    ...
    if (hasEditTools) {
      lines.addAll(zh
          ? [
              ...
              '只在创建新文件时使用 `write`，并且必须传入完整的 `content`；已有文件请改用 `edit` 或 `apply_patch`。',
              '',
              '# 文件引用（必须）',
          ...
          : [
              ...
              'Use `write` only for new files, and always provide the complete file body in `content`; for existing files, use `edit` or `apply_patch`.',
              '',
