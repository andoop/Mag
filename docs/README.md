...
|----------|----------|----------------|
| [Capabilities](capabilities.md) | Users / contributors | Product capability map: providers, MCP, Skills, tools, workspace, mobile runtime. |
...
- **Git behavior**: repository operations, credentials, SSH keys, branch/remote/sync behavior, platform bridge differences.
- **Mobile behavior**: Android floating window, iOS PiP behavior, background notifications, notification/overlay permissions, platform limitations.
...
| 文档 | 读者 | 内容 |
|------|------|------|
| [能力总览](capabilities.md) | 用户 / 贡献者 | 产品能力地图：模型供应商、MCP、Skills、工具、工作区、移动端运行时。 |
...
- **Git 行为**：仓库操作、凭据、SSH Key、分支/远程/同步行为、平台桥接差异。
- **移动端行为**：Android 小窗、iOS PiP、后台通知、通知/悬浮窗权限、平台限制。
