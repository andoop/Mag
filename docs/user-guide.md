...
- Attach workspace files, device files, photos, audio recordings, and video recordings when the action is enabled.
- Audio/video recording happens inside Mag and returns a temporary media attachment to the prompt.
...
| Camera and microphone | Enables in-app photo capture, audio recording, and video recording. |
...
- 输入提示词并发送，可创建或继续会话。
- 输入 `@` 可引用工作区文件（有可用建议时）。
- 在入口可用时，可附加工作区文件、设备文件、拍照、录音和录制视频。
- 录音/录像在 Mag 内完成，确认后会作为临时媒体附件交给 prompt。
