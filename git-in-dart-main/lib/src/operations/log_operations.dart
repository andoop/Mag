...
class LogOperation {
  ...
  Stream<GitCommit> logFrom(String ref, {LogOptions? options}) async* {
    final opts = options ?? const LogOptions();
    final commitHash = await repo.refs.resolveRef(ref);
  ...
  Future<List<GitCommit>> getCommitsBetween(
    ...
    final fromHash = await repo.refs.resolveRef(from);
    final toHash = await repo.refs.resolveRef(to);
  ...
  Future<GitCommit> getCommit(String ref) async {
    final hash = await repo.refs.resolveRef(ref);
